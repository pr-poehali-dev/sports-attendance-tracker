...
const Index = () => {
  ...
  const todayAttendance = athletes.filter(a => a.lastVisit === 'Сегодня').length;
  const avgAttendance = Math.round(athletes.reduce((acc, a) => acc + a.attendance, 0) / athletes.length);
        ...
        <Tabs defaultValue="athletes" className="w-full">
          ...
          <TabsContent value="athletes" className="space-y-3 mt-0">
            {athletes.map(athlete => (
              <Card 
                key={athlete.id} 
                className="p-4 transition-all hover:scale-[1.02] hover:shadow-lg border-l-4"
          ...
          <TabsContent value="groups" className="space-y-3 mt-0">
            {groups.map(group => (
              <Card key={group.id} className="p-5 transition-all hover:scale-[1.02] hover:shadow-lg">
